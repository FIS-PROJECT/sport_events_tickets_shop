...
public class TennisController implements Initializable {
    ...
    ObservableList<Event> list = FXCollections.observableList(db.readEvents().stream().filter(event -> event.getSportType().equals(SportType.TENNIS)).collect(Collectors.toList()));
    @Override
